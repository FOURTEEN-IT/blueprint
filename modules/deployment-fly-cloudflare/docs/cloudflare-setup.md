...
**Wichtiger Hinweis zur Herkunft:** Dieser Teil ist **nicht** aus Watchparty
extrahiert. Watchparty setzt bewusst **kein** Cloudflare ein — die
DNS-Subdomain liegt dort direkt bei IONOS (CNAME auf `*.fly.dev`), und
ADR-018 verwirft einen vorgeschalteten Proxy ausdruecklich:
...
bei Watchparty nicht vorliegen:
...
Wer keinen dieser Gründe hat, sollte wie Watchparty verfahren: DNS direkt
...
Watchparty, wo das Frontend-Build ins Backend-Jar gepackt wird, siehe
Modul `frontend-react-vite`). Für ein statisches React/Vite-Build, während
...
2. Umgebungsvariable für die Backend-URL setzen (z. B.
   ...
   Deployments gibt es keinen gemeinsamen Origin mehr wie im Watchparty-
   Modell.
...
Routing — das sind Erweiterungen für einen Bedarf, den weder Watchparty
noch die meisten aus diesem Blueprint entstehenden Projekte haben. Wer sie
