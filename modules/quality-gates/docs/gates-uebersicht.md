...
| Ebenen-Disjunktheit | Kein Adapter-/API-Test darf eine Domänenzeile abdecken, die kein unit-/port-Test selbst erreicht | eher `backend-java-onion` (hängt an dessen JaCoCo-Reports) | Als Konzept ja, hier nicht implementiert — Querverweis |
...
## Design-Prinzip dieses Moduls

> Ein übersprungener Skill ist kein Beinbruch, ein übersprungenes Gate gibt
> es nicht: Feature-Dokumentation, Anforderungsabdeckung (Backend und
> Frontend), Frontend-Tests, Protokollverträge, Architektur-Regeln,
> Mutationstests und der `commit-msg`-Hook greifen unabhängig davon, ob
> jemand einen Skill aufgerufen hat.
...
Die E2E-Ebene (`e2eTest`) hängt bewusst **nicht** an `check`, sondern läuft
als eigene Pipeline-Stufe vor dem Deploy — ein Browser-Durchlauf sprengt
sonst das Zehn-Minuten-Budget für `check`. Das ist kein übersprungenes
Gate, sondern ein Gate an anderer Stelle in der Pipeline.
