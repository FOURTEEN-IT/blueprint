...
Tippspiel-Modul in Watchparty (`adapter/out/db`, `config/league/LeagueDatabaseConfig`,
...
- Konfiguration (DataSource + Flyway-Wiring) landet in `config/` (bzw. einem
  ...
  aufteilt, wie Watchparty es mit `config/league` tut).

## Platzhalter

| Platzhalter | Bedeutung | Beispiel aus Watchparty |
|---|---|---|
| `{{PACKAGE_BASE}}` | Basis-Package des Zielprojekts | `de.fourteen.watchparty` |
| `{{app}}` | Präfix für Spring-Properties | `watchparty` (→ `watchparty.league.db.url`) |
...
`{{app}}.db.url` (Watchparty: `watchparty.league.db.url`).
...
Hauptanwendungsklasse des Zielprojekts abzuschalten (bei Watchparty über
`exclude` in `@SpringBootApplication`) — sonst konkurriert sie mit dem
...
- Ob der Generator-Skill `{{DB_SCHEMA_LABEL}}` immer setzt oder auch ein
  ...
  Varianten kommen in Watchparty nicht nebeneinander vor (dort gibt es nur
  `db/league/migration`, weil es bislang nur einen Datenbank-nutzenden
  Fachbereich gibt).
- Ob Nutzername/Passwort immer als eigene Properties existieren oder auch
  vollständig in der JDBC-URL stehen dürfen (wie es `LeagueDatabaseConfig`
...
- Dieses Modul nimmt keine Aussage zu Connection-Pool-Feineinstellungen
  (Pool-Größe, Timeouts) vor, weil Watchparty selbst keine über die
  Hikari-Defaults hinaus setzt.
