...
| [`deployment-fly`](../modules/deployment-fly/README.md) | Fly.io-Hosting (eine Instanz), Dockerfile, GitHub-Actions-CI/CD, Semantic Release | — (unabhängig, aber sinnvoll erst mit einem fertig bauenden Projekt) |
...
| [`dependabot-maintenance`](../modules/dependabot-maintenance/README.md) | Dependabot-Konfiguration, CodeQL, OpenRewrite für Major-Sprünge, die tägliche Routine, die offene Dependabot-PRs sichtet und mergt | `backend-java-onion` (praktisch), optional `frontend-react-vite`/`quality-gates`/`deployment-fly` |
...
4. **`deployment-fly`** zuletzt, wenn ein Projekt lokal baut und
